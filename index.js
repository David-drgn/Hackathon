...
app.post("/api/sendMail", async(req, res) => {
    try {
        ...
        res.json({
            erro: false,
            message: "Código enviado com sucesso!! Verifique a sua caixa de email, lembre que o acesso é permitido apenas após 5 minutos",
        });
    } catch {
        return res.redirect("/systemErro?erro=400");
    }
...
app.post("/api/relationCreate", async(req, res) => {
    try {
        const connect = await new Connect();
        await connect.initialize();
        ...
        const response = await service.createRelation(req.body.accountId, req.body.serviceId);
